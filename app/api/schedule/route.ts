...
const client = wrapper(axios.create({ jar, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" } }));
const urlLogin = "http://220.231.119.171/kcntt/login.aspx";

function tinhtoan(tiethoc: string) {
  if (!tiethoc.includes(' --\u003E ')) return undefined;
...
function lichtuan(lich: string) {
  if (typeof lich !== 'string') return { Tu: "1970-1-1", Den: "1970-1-1" };
...
function thutrongtuan(thu: string, batdau: string, ketthuc: string) {
  if (typeof thu !== 'string' || typeof batdau !== 'string' || typeof ketthuc !== 'string') return "Invalid input";
  ...
  if (thuIndex < 2 || thuIndex > 8 || isNaN(thuIndex)) return "Invalid weekday number";
...
export const GET = async (request: Request) => {
  ...
  const getAllFormElements = (element: HTMLFormElement) => Array.from(element.elements).filter(tag => ["select", "textarea", "input"].includes(tag.tagName.toLowerCase()) && tag.getAttribute("name"));
  const body = new URLSearchParams();
  getAllFormElements(DOMsession.window.document.getElementById("Form1") as HTMLFormElement).forEach((_: any) => {
    const key = _.getAttribute("name");
    let value = _.getAttribute("value");
    if (key == "txtUserName") value = username;
    else if (key == "txtPassword") value = password ? (ishash == "true" ? password : createHash("md5").update(password).digest("hex")) : "";
  ...
  if (errorInfo && errorInfo.textContent !== "") {
    return new Response(JSON.stringify({ error: true, message: errorInfo.textContent, msv: username, pwd: password }), { headers: { "content-type": "application/json" } });
  }
  ...
  hiddenFields.forEach(input => hiddenValues[(input as HTMLInputElement).name] = (input as HTMLInputElement).value);
  ...
  if (studentInfo && studentInfo?.textContent) {
    let studenId = studentInfo.textContent.split("(")[1].split(")")[0];
    let studentname = studentInfo.textContent.split("(")[0];
    const sinhvien = { MaSV: studenId, TenSV: studentname };
    const hockiELM = document.getElementById("drpSemester") as HTMLSelectElement;
    let hocki = hockiELM.options[hockiELM.selectedIndex].text.split("_")[0];
    const namhoc = `${hockiELM.options[hockiELM.selectedIndex].text.split("_")[1]} - ${hockiELM.options[hockiELM.selectedIndex].text.split("_")[2]}`;
    const exportResponse = await client.post(DOMurl, new URLSearchParams({ ...hiddenValues, drpSemester: semester, drpTerm: term, drpType: type, btnView: btnView }).toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, responseType: 'arraybuffer' });
    ...
    for (let i = 10; i < jsonData.length; i++) {
      ...
      if (typeof LetTime !== 'undefined') {
        ...
        LichHoc.push({ STT, Tuan, Ngay, TenHP, MaHP, ThoiGian: LetTime, GiangVien: gv[0], Meet: gv[1], DiaDiem });
      } else {
        Tuan++;
        const NgayHoc = lichtuan(hocphan.match(/\((.*?)\)/)[1]);
        ngayhoct = NgayHoc;
        TuanData.push({ Tuan, NgayHoc });
      }
    }
    return new Response(JSON.stringify({ HocKi: hocki, NamHoc: namhoc, SinhVien: sinhvien, TuanData: TuanData, LichHoc: LichHoc }), { headers: { "content-type": "application/json" } });
  } else {
    return new Response(JSON.stringify({ error: true, message: "Sai mã sinh viên hoặc mật khẩu", msv: username, pwd: password }));
  }
};
